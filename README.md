...
### Supabase = ที่เก็บข้อมูลดิบ · Mixhub = ที่แยกข้อมูล (17 ก.ย. 69)

ทุก request ที่ Meta ส่งมา เก็บลง `raw_events` **ทั้งก้อนก่อนแยกอะไรทั้งนั้น** (กลบเบอร์/อีเมลในข้อความ
ส่วน ID/เวลาไม่แตะ) แล้ว `sync_to_mixhub.py` ดึงไปลง `mixhub.fbchat_raw_events`

| | ก่อน | ตอนนี้ |
|---|---|---|
| event ที่โค้ดไม่รู้จัก (อ่านแล้ว รีแอคชัน แก้ข้อความ คอมเมนต์ object อื่น) | ทิ้ง | เก็บ |
| แยกข้อมูลพลาด | ตอบ 500 ให้ Meta ยิงซ้ำ (ถ้ายังพลาดคือหาย) | ตอบ 200 เพราะก้อนดิบลงฐานแล้ว Mixhub แยกเองได้ |
| เก็บก้อนดิบไม่สำเร็จ | — | ตอบ 500 ให้ Meta ยิงซ้ำ |
| Meta ยิงซ้ำก้อนเดิม | กันด้วย mid | กันด้วย sha256 ของไบต์ดิบ |

`/api/export` ส่ง `raw_events` ด้วย cursor แยก (`since_raw`) เพราะ seq คนละลำดับกับตารางอื่น
`/api/prune` ลบก้อนดิบด้วยกติกาเดียวกัน (ยืนยันแล้วว่าเข้า mixhub + เก่ากว่า N วัน)
...
### เก็บดิบ ตัดสินทีหลัง (17 ก.ย. 69)

`actor` ในฐาน Supabase เป็น **ค่าชั่วคราว** ใช้แค่ `/api/health` — ป้ายจริงตัดสินที่ Mixhub
จาก `raw` + ตาราง `mixhub.fbchat_apps` ที่คนแก้ได้ (`sync_to_mixhub.py` ไม่ลอก `actor` จากที่นี่)

ทำไม: วันแรกหลัง live ข้อมูลจริงออกมาว่า "คน" ไม่ขยับเลย "AI" ค้างที่ 1 แต่ "บอท" (app_id
ที่ไม่รู้จัก) เพิ่มเร็วสุด — ตารางข้างบนเขียนก่อนเคยเห็นข้อมูลจริง ถ้าป้ายตายตัวตอนรับเข้า
รายงานผิดแล้วแก้ย้อนหลังไม่ได้

```bash
php legacy/tools/chat-analysis/fbchat_apps.php                       # ดูแอปที่ยังไม่รู้จัก + ตัวอย่างข้อความ
php legacy/tools/chat-analysis/fbchat_apps.php set <app_id> ai "ชื่อ"   # กำหนดแล้วตัดสินแถวเก่าใหม่ทันที
```
(รันใน repo Mixhub)
